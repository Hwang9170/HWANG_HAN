...
const getNearestCafe = (cafes, preferences) => {
    let nearestCafe = null;
    let minDiff = Infinity;

    cafes.forEach(cafe => {
        const diff = Math.abs(cafe.ratings.facility - preferences.facility) +
                     Math.abs(cafe.ratings.noise - preferences.noise) +
                     Math.abs(cafe.ratings.taste - preferences.taste);
...
const recommendCafe = (location, preferences) => {
    ...
    if (filteredCafes.length > 0) {
        ...
        return getNearestCafe(cafes, preferences);
    }
};
